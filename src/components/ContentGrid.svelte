...
import Card from "./Card.svelte";
  ...
  function generateLayout(col) {
    return new Array(10).fill(null).map(function (item, i) {
      ...
      return {
        [col]: gridHelp.item({ x: (i * 2) % col, y: Math.floor(i / 6) * y, w: 1, h: y }),
        id: id(),
...
<Grid bind:items {cols} rowHeight={100} let:item={item} fillSpace={true} fastStart={true} on:change={e => console.log("hello", e)}>
	<pre>
